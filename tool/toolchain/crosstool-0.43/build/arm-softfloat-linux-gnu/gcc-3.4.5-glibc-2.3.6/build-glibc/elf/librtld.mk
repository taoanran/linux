rtld-csu +=check_fds.os
rtld-csu +=errno-loc.os
rtld-csu +=divdi3.os
rtld-setjmp +=bsd-_setjmp.os
rtld-setjmp +=__longjmp.os
rtld-stdlib +=mp_clz_tab.os
rtld-string +=strchr.os
rtld-string +=strcmp.os
rtld-string +=strlen.os
rtld-string +=strnlen.os
rtld-string +=memchr.os
rtld-string +=memcmp.os
rtld-string +=memmove.os
rtld-string +=memset.os
rtld-string +=mempcpy.os
rtld-string +=stpcpy.os
rtld-string +=memcpy.os
rtld-string +=wordcopy.os
rtld-posix +=uname.os
rtld-posix +=_exit.os
rtld-posix +=getpid.os
rtld-posix +=environ.os
rtld-io +=xstat64.os
rtld-io +=fxstat64.os
rtld-io +=open.os
rtld-io +=close.os
rtld-io +=read.os
rtld-io +=write.os
rtld-io +=lseek.os
rtld-io +=access.os
rtld-io +=fcntl.os
rtld-io +=getcwd.os
rtld-misc +=getpagesize.os
rtld-misc +=mmap.os
rtld-misc +=munmap.os
rtld-misc +=mprotect.os
rtld-gmon +=profil.os
rtld-gmon +=prof-freq.os
rtld-linuxthreads +=forward.os
rtld-linuxthreads +=libc-cancellation.os
rtld-csu +=sysdep.os
rtld-csu +=errno.os
rtld-setjmp +=setjmp.os
rtld-signal +=sigaction.os
rtld-signal +=sigrestorer.os
rtld-stdlib +=exit.os
rtld-stdlib +=cxa_atexit.os
rtld-time +=setitimer.os
rtld-subdirs = csu gmon io linuxthreads misc posix setjmp signal stdlib string time
